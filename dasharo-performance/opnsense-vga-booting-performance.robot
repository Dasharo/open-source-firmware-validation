...
BOV001.503 Boot OPNSense (VGA output) from disk after cold-boot (OPNSense)
    [Documentation]    This test aims to verify that OPNSense (VGA output)
    ...
    Execute Manual Step
    ...    [4/5] In the Boot Menu, select the disk on which OPNSense VGA was previously installed or boot entry with the name of OPNSense.
    Execute Manual Step    [5/5] Confirm that the OPNSense login screen is displayed via VGA output.
...
BOV002.503 Boot OPNSense (VGA output) from disk after warm-boot (OPNSense)
    ...
    Execute Manual Step
    ...    [3/4] In the Boot Menu, select the disk on which OPNSense VGA was previously installed or boot entry with the name of OPNSense.
    Execute Manual Step    [4/4] Confirm that the OPNSense login screen is displayed via VGA output.
...
BOV003.503 Boot OPNSense (VGA output) from disk after reboot (OPNSense)
    ...
    Execute Manual Step
    ...    [5/6] In the Boot Menu, select the disk on which OPNSense VGA was previously installed or boot entry with the name of OPNSense.
    Execute Manual Step    [6/6] Confirm that the OPNSense login screen is displayed via VGA output.
