...
DUB001.002 USB devices recognition (Ubuntu 22.04)
    ...
    Skip If    not ${docking_station_usb_support}    DUB001.002 not supported
    Skip If    not ${tests_in_ubuntu_support}    DUB001.002 not supported
    Power On
    Login to Linux
    Switch to root user
...
DUB001.003 USB devices recognition (Windows 11)
    ...
    Skip If    not ${docking_station_usb_support}    DUB001.003 not supported
    Skip If    not ${tests_in_windows_support}    DUB001.003 not supported
    Power On
    Login to Windows
...
DUB002.002 USB keyboard detection (Ubuntu 22.04)
    ...
    Skip If    not ${docking_station_keyboard_support}    DUB002.002 not supported
    Skip If    not ${tests_in_ubuntu_support}    DUB002.002 not supported
    Power On
    Login to Linux
    Switch to root user
    ${out}=    List devices in Linux    usb
...
DUB002.003 USB keyboard detection (Windows 11)
    [Documentation]    Check whether the external USB keyboard connected to the
    ...    docking station is detected correctly by the Windows OS.
    Skip If    not ${docking_station_keyboard_support}    DUB002.003 not supported
    Skip If    not ${tests_in_windows_support}    DUB002.003 not supported
    Power On
    Login to Windows
...
DUB003.001 Upload 1GB file on USB storage (Ubuntu 22.04)
    ...
    ...    operating system to the USB storage connected to the
    ...    docking station.
    IF    not ${docking_station_upload_support}
        SKIP    DUB003.001 not supported
    END
    IF    not ${tests_in_ubuntu_support}    SKIP    DUB003.001 not supported
    Power On
    Login to Linux
    Switch to root user
...
DUB003.002 Upload 1GB file on USB storage (Windows 11)
    ...
    ...    docking station.
    IF    not ${docking_station_upload_support}
        SKIP    DUB003.002 not supported
    END
    IF    not ${tests_in_windows_support}    SKIP    DUB003.002 not supported
    Power On
    Login to Windows
