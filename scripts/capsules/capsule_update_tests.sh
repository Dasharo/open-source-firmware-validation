...
echo "--- CREATING CAPSULE WITH SERIAL REDIRECTION ---"

file_descriptor="_serial_redirection"
output_file="$capsule_name$file_descriptor.json"
...
cd ../.. || exit
cd dcu || exit
cp ../dl-cache/edk2/$payload ./
./dcuc variable ./$payload --set SerialRedirection --value Enabled
cd ../dl-cache/edk2 || exit
cp ../../dcu/$payload ./
