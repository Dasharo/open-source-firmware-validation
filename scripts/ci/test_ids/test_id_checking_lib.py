...
def check_mappings():
    return(len(compare_mappings())==0)

def compare_mappings():
    robot = subprocess.run('./scripts/list-tests-from-robot.sh', capture_output=True, text=True)
    json = subprocess.run('./scripts/list-tests-from-json.sh', capture_output=True, text=True)
