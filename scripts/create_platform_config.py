...
PLATFORM_CONFIGS_PATH = os.path.dirname(os.path.realpath(__file__)) + \
    "/../platform-configs"
...
if __name__ == "__main__":
    ...
    print(f"Creating new config {new_config_name} with includes {vendor_includes_names}")
    ...
    undefined_variables = [var for var in default_variable_pairs 
        if var[1] == UNDEFINED_VARIABLE_VALUE and 
        var[0] not in [var[0] for var in includes_variable_pairs]
    ]
    ...
    if undefined_count > 0:
        print(f"Warning: {undefined_count} variables are not defined in any of the includes"
              " and need attention!")
