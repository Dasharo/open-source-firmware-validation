...
_PARALLEL_Background Measurements Immediate (no load) (Ubuntu)
    ...
    ${will_any_be_run}=    Will Parallel Test Be Run Regex    CPF001.201
...
CPF001.201 CPU not stuck on initial frequency (Ubuntu)
    ...
    @{frequencies}=    Get Parallel Test Outputs    ${PARALLEL_TEST_ID}
...
_PARALLEL_Background Measurements (no load) (Ubuntu)
    ...
    Set Variable If    ${gather_temps}    ${temperature_test_measure_interval}    999999
    Set Variable If    ${measure_stab}    ${stability_test_measure_interval}    999999
    Set Variable If    ${gather_freqs}    ${frequency_test_measure_interval}    999999
    ...
    Set Parallel Test Outputs    CPT001.201    ${temp_list}
    Set Parallel Test Outputs    CPF005.201    ${freq_list}
...
Prepare Parallel Test Suite
    ...
    Add Parallel Test Skip Condition    not ${TESTS_IN_UBUNTU_SUPPORT}    tests in Ubuntu not supported
    Add Parallel Test Skip Condition    '${ENV_ID_UBUNTU}' not in ${TESTED_LINUX_DISTROS}    Ubuntu not in tested distros
    Add Parallel Test Skip Condition    ${LAPTOP_PLATFORM}    The Platform is a Laptop
