...
Boot System Or From Connected Disk
    ...
    ...
    ...    The firmware-specific part (entering the boot menu, matching the OS
    ...    entry, falling back to a connected disk) is delegated to the payload
    ...    library selected via ${PAYLOAD_LIB} (``Payload Select Boot Device``).
    ...    The common prologue below (setting ${BOOTED_OS_ID}, loading OS
    ...    credentials, and the SSH short-circuit) is payload-agnostic.
    ...
    ...    === Requirements ===
    ...    - Has to be called in quick succession after powering on or rebooting in order to
    ...    \ react before the auto boot time-out passes
